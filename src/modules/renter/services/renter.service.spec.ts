...
describe('RenterService', () => {
  ...
  describe('create', () => {
    ...
    it('should create a new renter', async () => {
      ...
      expect(result).toEqual(mockRenter);
      expect(mockRenterRepository.findByEmail).toHaveBeenCalledWith(
        'john@example.com',
      );
    ...
    it('should throw ConflictException when email already exists', async () => {
      ...
      await expect(service.create(createRenterDto)).rejects.toThrow(
        ConflictException,
      );
    });
  });
  ...
  describe('update', () => {
    ...
    it('should update a renter', async () => {
      mockRenterRepository.findById.mockResolvedValue(mockRenter);
      mockRenterRepository.findByEmail.mockResolvedValue(null);
      mockRenterRepository.update.mockResolvedValue({
        ...mockRenter,
        ...updateRenterDto,
      });
    ...
    it('should throw EntityNotFoundException when renter not found', async () => {
      ...
      await expect(service.update(1, updateRenterDto)).rejects.toThrow(
        EntityNotFoundException,
      );
    });
    ...
    it('should throw ConflictException when new email already exists', async () => {
      mockRenterRepository.findById.mockResolvedValue(mockRenter);
      mockRenterRepository.findByEmail.mockResolvedValue({
        ...mockRenter,
        id: 2,
      });
  ...
  describe('findByType', () => {
    it('should return renters by type', async () => {
      ...
      expect(result).toEqual(renters);
      expect(mockRenterRepository.findByType).toHaveBeenCalledWith(
        'individual',
      );
    });
  });
